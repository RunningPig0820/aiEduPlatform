# admin-class-management Specification

## Purpose
行政班管理能力：在 `t_department` 上以 `department_type` 区分 `ORG`（普通组织部门）与 `ADMIN_CLASS`（行政班节点），通过 `t_department_edu` 表反向关联存储学段/年级/班级的教育属性（stage_code、stage_year_code、grade_code、enrollment_year），复用 `parent_id` 与 `department_path` 构建学段 → 年级 → 班级树，并提供行政班节点 CRUD API。
