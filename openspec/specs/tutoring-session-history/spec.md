# tutoring-session-history Specification

## Purpose
会话历史管理能力：学生可查看本人全部状态（进行中/已归档/已终止）的历史答疑会话列表、删除历史会话（软删除）、查看会话详情；会话消息携带工作流 meta 字段，经 Redis 热存与 COS transcript 持久化，供前端从 COS 复原历史工作流快照。
