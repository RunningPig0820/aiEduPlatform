# tutoring-agent-events Specification

## Purpose
答疑 SSE 事件契约能力：Java 以流式方式消费 Python decide 响应（过滤 `meta` 事件解析 ActionMeta），透传 generate 流的 `agent` 事件，并注入 Java 侧的 `guardrail`、`memory` 事件，形成标准 agent 事件格式与阶段表，供前端按协议渲染答疑阶段。
