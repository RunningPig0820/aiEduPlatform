...
- 新增知识点导航 API：支持教材→章节→小节→知识点逐级浏览，所有路径参数使用 URI
...
- `kg-navigation`: 知识点导航查询能力，支持按教材→章节→小节→知识点逐级浏览，以及按学科/年级筛选
...
- **Database 新增表**: 4 张节点主表（uri 主键）+ 3 张层级关联表 + 1 张同步记录表
- **Backend 新增 API**: `/api/kg/**` 接口组（同步 + 导航 + 知识体系）
