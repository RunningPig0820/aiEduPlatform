...
题型名/题目动态聚集——字符级规则 + 向量库最近邻（题目向量为主）→ canonical；**零锚点动态涌现**（canonical 由第一条相似题创建，无题库也能跑通），掌握表 key 源头不裂行。
...
### Requirement: 向量库最近邻聚集（题目向量为主）

系统 SHALL 在字符规则未命中后使用向量库查最近邻：**题目文本向量为主**（同题型强信号，题面稳定）+ **题型名向量为辅**（同题型不同题面），top-1 相似度 ≥ 阈值 → 归一到该 canonical 并写别名表。双命中高置信直接并，单命中进候选。

#### Scenario: 题目向量命中归并
- **WHEN** 新题题目文本与已有题向量相似度 ≥ 阈值（连发「鸡兔同笼」变体），题型名不同
- **THEN** 系统归一到已有 canonical，并写别名表（新题型名 → canonical）
...
#### Scenario: 相似度不足不聚集
- **WHEN** 新题与最近邻相似度 < 阈值（双信号均不足）
- **THEN** 系统不归并，走建新 canonical 路径

### Requirement: 未命中建新 canonical（无锚点冷启动）

系统 SHALL 在规则与向量最近邻均未命中时，将题型名作为新 canonical 建题型并入库向量（题目向量 + 题型名向量），供后续聚集。**无题库/无预置分类时锚点由第一条相似题动态创建。**

#### Scenario: 首题建锚
- **WHEN** 学生第 1 条题无任何近邻（向量桶空）
- **THEN** 系统创建新 canonical（首见题型名），入库题目向量 + 题型名向量

#### Scenario: 新题型建库
- **WHEN** 题型名既无字符规则命中、向量库亦无 ≥ 阈值最近邻
- **THEN** 系统创建新 canonical 题型，并 embedding 入库向量

### Requirement: 批量聚集兜底

系统 SHALL 提供定时批量聚集：扫描题目表未归并/低置信题型名，全量向量聚类补归并，canonical 名以首见名/最高频名兜底（可定时 LLM 归纳规范名），重算掌握表聚合。

#### Scenario: 散名批量归并
- **WHEN** 历史题目表存在多个近义散名（「鸡兔同笼/鸡兔同笼问题/假设法」）未归并
- **THEN** 批量聚类把它们并到同一 canonical，重算掌握表

#### Scenario: canonical 规范名归纳
- **WHEN** 批量聚集完成后组内有多个成员名
- **THEN** 系统以首见名/最高频名兜底，可定时用 LLM 归纳规范名
