...
@Mapper
@DS("kg")
public interface KgSectionMapper extends BaseMapper<KgSectionPo> {

    @Select("SELECT * FROM t_kg_section WHERE uri = #{uri} AND is_deleted = false")
    KgSectionPo selectByUri(@Param("uri") String uri);
    ...
    @Select("<script>" +
            ...
            "</script>")
    List<KgSectionPo> selectByUris(@Param("uris") List<String> uris);

    @Select("SELECT * FROM t_kg_section WHERE status = #{status} AND is_deleted = false")
    List<KgSectionPo> selectByStatus(@Param("status") String status);
