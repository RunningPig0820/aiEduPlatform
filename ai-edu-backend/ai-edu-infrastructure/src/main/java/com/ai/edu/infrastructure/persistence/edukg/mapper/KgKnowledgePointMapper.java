...
public interface KgKnowledgePointMapper extends BaseMapper<KgKnowledgePointPo> {
    ...
    /** 按学段取全部知识点 label（去重，封闭域约束选择候选池）。 */
    @Select("SELECT DISTINCT kp.label FROM t_kg_textbook tb " +
            "JOIN t_kg_textbook_chapter tc ON tc.textbook_uri = tb.uri AND tc.is_deleted = false " +
            "JOIN t_kg_chapter ch ON ch.uri = tc.chapter_uri AND ch.is_deleted = false " +
            "JOIN t_kg_chapter_section cs ON cs.chapter_uri = ch.uri AND cs.is_deleted = false " +
            "JOIN t_kg_section sec ON sec.uri = cs.section_uri AND sec.is_deleted = false " +
            ...
            "JOIN t_kg_knowledge_point kp ON kp.uri = skp.kp_uri AND kp.is_deleted = false " +
            "WHERE tb.stage = #{stage} AND tb.is_deleted = false")
    List<String> selectLabelsByStage(@Param("stage") String stage);

    /** 按学段 + 关键词分页列知识点（label LIKE，可选；keyword 空则全量）。 */
    @Select("<script>SELECT kp.uri AS kpUri, kp.label AS kpLabel, tb.stage AS stage, ch.label AS chapterLabel, sec.label AS sectionLabel " +
            "FROM t_kg_textbook tb " +
            "JOIN t_kg_textbook_chapter tc ON tc.textbook_uri = tb.uri AND tc.is_deleted = false " +
            "JOIN t_kg_chapter ch ON ch.uri = tc.chapter_uri AND ch.is_deleted = false " +
            "JOIN t_kg_chapter_section cs ON cs.chapter_uri = ch.uri AND cs.is_deleted = false " +
            ...
            "JOIN t_kg_knowledge_point kp ON kp.uri = skp.kp_uri AND kp.is_deleted = false " +
            "WHERE tb.stage = #{stage} AND tb.is_deleted = false " +
            "<if test='keyword != null and keyword != \"\"'> AND kp.label LIKE CONCAT('%', #{keyword}, '%')</if> " +
            "GROUP BY kp.uri, kp.label, tb.stage, ch.label, sec.label " +
            "ORDER BY kp.uri LIMIT #{limit} OFFSET #{offset}</script>")
    List<KgKpPlacement> selectPageByStageAndKeyword(@Param("stage") String stage, @Param("keyword") String keyword,
                                                    @Param("offset") int offset, @Param("limit") int limit);

    /** 某学段 + 关键词知识点总数。 */
    @Select("<script>SELECT COUNT(DISTINCT kp.uri) " +
            "FROM t_kg_textbook tb " +
            "JOIN t_kg_textbook_chapter tc ON tc.textbook_uri = tb.uri AND tc.is_deleted = false " +
            "JOIN t_kg_chapter ch ON ch.uri = tc.chapter_uri AND ch.is_deleted = false " +
            "JOIN t_kg_chapter_section cs ON cs.chapter_uri = ch.uri AND cs.is_deleted = false " +
            ...
            "JOIN t_kg_knowledge_point kp ON kp.uri = skp.kp_uri AND kp.is_deleted = false " +
            "WHERE tb.stage = #{stage} AND tb.is_deleted = false " +
            "<if test='keyword != null and keyword != \"\"'> AND kp.label LIKE CONCAT('%', #{keyword}, '%')</if></script>")
    long countByStageAndKeyword(@Param("stage") String stage, @Param("keyword") String keyword);
}
