...
@Mapper
@DS("kg")
public interface KgChapterMapper extends BaseMapper<KgChapter> {

    @Select("SELECT * FROM t_kg_chapter WHERE uri = #{uri} AND is_deleted = false")
    KgChapter selectByUri(@Param("uri") String uri);
    ...
    @Select("<script>" +
            ...
            "</script>")
    List<KgChapter> selectByUris(@Param("uris") List<String> uris);

    @Select("SELECT * FROM t_kg_chapter WHERE status = #{status} AND is_deleted = false")
    List<KgChapter> selectByStatus(@Param("status") String status);
